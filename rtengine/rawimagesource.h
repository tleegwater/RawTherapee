...
protected:
    ...
    void border_interpolate(unsigned int border, float (*tile)[4], unsigned int start = 0, unsigned int end = 0);
    void border_interpolate2(int winw, int winh, int lborders);
    void dcb_initTileLimits(int &colMin, int &rowMin, int &colMax, int &rowMax, int x0, int y0, int border, int borderOffset = 4);
    void dcb_fill_raw( float (*tile )[3], int x0, int y0, float** rawData);
    void dcb_fill_border( float (*tile )[3], int border, int x0, int y0);
    void dcb_hid(float (*tile)[3], int x0, int y0);
    void dcb_color(float (*tile)[3], int x0, int y0);
    void dcb_hid2(float (*tile)[3], int x0, int y0);
    void dcb_map(float (*tile)[3], uint8_t *map, int x0, int y0);
    void dcb_correction(float (*tile)[3], uint8_t* const map, int x0, int y0);
    void dcb_pp(float (*tile)[3], int x0, int y0);
    void dcb_correction2(float (*tile)[3], uint8_t *map, int x0, int y0);
    void dcb_restore_red_blue(float (*tile)[3], int x0, int y0, float** rawData);
    void dcb_refinement(float (*tile)[3], uint8_t *map, int x0, int y0);
    void dcb_color_full(float (*tile)[3], int x0, int y0, float (*chroma)[2]);
